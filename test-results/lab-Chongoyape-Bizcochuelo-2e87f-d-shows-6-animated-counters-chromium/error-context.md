...
- Name: lab.spec.ts >> Chongoyape Bizcochuelos Lab — full functional validation >> lab stats band shows 6 animated counters
- Location: tests/lab.spec.ts:46:7
...
Locator: getByText('Validation rounds')
Expected: visible
Error: strict mode violation: getByText('Validation rounds') resolved to 5 elements:
    1) <span class="font-medium uppercase tracking-wide">Validation rounds passed</span> aka getByText('Validation rounds passed')
    2) <div class="text-xs font-semibold text-foreground mt-0.5 leading-tight">Validation rounds</div> aka getByText('Validation rounds', { exact: true })
    3) <div data-slot="card-content" class="px-6 text-xs text-amber-900/80 dark:text-amber-200/80 leading-relaxed">…</div> aka getByText('Kitchen validation rounds are documented as predicted results with explicit')
    4) <dd class="text-[11px] text-muted-foreground leading-relaxed mt-0.5">The protocol requires two consecutive "quiet" val…</dd> aka getByText('The protocol requires two')
    5) <span>…</span> aka getByText('Kitchen validation rounds are labelled predicted — not executed in this')

Call log:
  - Expect "toBeVisible" with timeout 10000ms
  - waiting for getByText('Validation rounds')
